...
import { resetSyncInventoryTaskId } from "redux/tasks/tasksSlice";
...
const SyncingInventorySnackbar = forwardRef(({ id, ...props }, ref) => {
  ...
  const handleFetchTaskStatus = () => {
    ...
    const URL = INVENTORY.PULL_PUSH_INVENTORY_STATUS;
    const data = {
      user_id: currentUser.merchant_id,
      task_id: syncInventoryTaskId,
    };
    if (currentUser) {
      appFetch(URL, data)
        .then((json) => {
          if (json.status === API_RESPONSE_STATUS.SUCCESS) {
            if (json.result.status === API_RESPONSE_STATUS.SUCCESS) {
              // closeSnackbar(id);
              handleDismiss();
              enqueueSnackbar("Syncing inventory completed", {});
              setSyncing(json.result.status);
              dispatch(resetSyncInventoryTaskId());
            } else {
              if (!currentUser) return;
              setTimeout(() => {
  ...
  return (
    <SnackbarContent ref={ref}>
      ...
      >
        ...
        >
          {/* {console.log({ syncing: syncing === "in_progress" })} */}
          <div className="refresh-icon">
            ...
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              className={syncing === "in_progress" && " rotate-icon"}
            >
              <path
          ...
          <Stack sx={{ flex: 1, ml: 1 }}>
            ...
            >
              Syncing Inventory data from all the stores.
            </Typography>
          </Stack>
            ...
            {Array.isArray(data) &&
              data.map((item, index) => {
                return (
                  ...
                  >
                    <RenderAppImage
                      display_image={channelsOptions[item?.channel_id]?.image}
                    />
                    <Box sx={{ flex: 1, ml: 2 }}>
                      <Typography
                        sx={{
                      ...
                      >
                        {item.completed}/{item.total} Products
                      </Typography>
...
SyncingInventorySnackbar.displayName = "SyncingInventorySnackbar";

export default SyncingInventorySnackbar;
